<template>
  <div class="wrap" :style="`width:${size}px;`">
    <div class="selectInput">
      ...
      <icon type="ios-arrow-down" size="18" class="icon_drop" :class="listShow?'icon_drop_active':''"
            :on-click="showListHandle"></icon>
    </div>
    <div class="list" @changeTitle="titleHandle" :class="listShow?'showListactive':''">
      <p v-for="(item,idx) in listData" :key="idx" @click="getTitleHandle(item,idx)" :class="idx===active?'active':''">
        {{item.label}}</p>
    </div>
  </div>
</template>
